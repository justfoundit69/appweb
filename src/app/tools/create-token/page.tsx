    ...
    <RequireWallet>
      <div className="min-h-screen py-8 relative">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="mb-8">
            <h1 className="text-3xl font-bold text-[#1C180D] mb-2">Create Token</h1>
            ...
            <>
              <div className="fixed top-16 left-0 right-0 lg:left-64 bottom-0 z-40 pointer-events-auto cursor-not-allowed select-none">
                ...
                <div className="absolute inset-x-0 top-1/2 -translate-y-1/2 mx-auto max-w-2xl z-20 text-center px-6">
                  <div className="rounded-lg border border-[#1C180D]/20 bg-white backdrop-blur-sm p-4 shadow-sm">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 relative">
            ...
            <div className={`lg:col-span-8 ${isComingSoon ? 'blur-sm select-none pointer-events-none user-select-none' : ''}`}>
              <div className="card p-8">
                ...
                {isSuccess && hash && (
                  <div className="mt-8 p-4 bg-white border border-[#1C180D]/30 rounded-lg">
                    <h3 className="text-lg font-semibold text-[#1C180D] mb-2">Token Created Successfully!</h3>
            ...
            <div className={`lg:col-span-4 ${isComingSoon ? 'blur-sm select-none pointer-events-none user-select-none' : ''}`}>
              <div className="card p-6 lg:sticky lg:top-24 space-y-6">
                {/* Fee Information */}
                <div className="bg-white backdrop-blur-sm border border-[#1C180D]/15 rounded-lg p-4">
                  <h4 className="font-medium text-[#1C180D] mb-2">Token Creation Fee</h4>
                  <p className="text-sm text-[#3f3a2b]">
                ...
                <div>
                  <h3 className="text-lg font-semibold text-[#1C180D] mb-2">Preview</h3>
                  <div className="bg-white backdrop-blur-sm border border-[#1C180D]/15 rounded-lg p-4 space-y-2">
                    <p className="text-sm text-[#3f3a2b]">Name</p>
                    <p className="font-semibold text-[#1C180D]">{watch('name') || '—'}</p>
                    <p className="text-sm text-[#3f3a2b] mt-3">Symbol</p>
                    <p className="font-semibold text-[#1C180D]">{watch('symbol') || '—'}</p>
                    <p className="text-sm text-[#3f3a2b] mt-3">Decimals</p>
                    <p className="font-semibold text-[#1C180D]">{watch('decimals') || '18'}</p>
                    <p className="text-sm text-[#3f3a2b] mt-3">Total Supply</p>
                    <p className="font-semibold text-[#1C180D]">{watch('totalSupply') || '—'}</p>
                    <p className="text-sm text-[#3f3a2b] mt-3">Owner</p>
                    <p className="font-mono text-[#1C180D] break-all">{watch('owner') || '—'}</p>
                  </div>
                </div>
                <div>
                  <h3 className="text-lg font-semibold text-[#1C180D] mb-2">Guidelines</h3>
